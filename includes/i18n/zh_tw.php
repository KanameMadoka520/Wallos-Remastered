...
$i18n = [
    ...
    "user_group_management_info" => "只有管理員可以調整使用者分組。Free 使用者只能使用圖片外鏈，受信使用者還可以為訂閱詳情上傳託管到伺服器的圖片。",
    "subscription_images" => "訂閱圖片",
    "subscription_image_upload" => "上傳伺服器圖片",
    ...
    "subscription_image_limits_notice" => "伺服器託管圖片單張上限 10 MB，僅支援 JPEG、PNG、WebP。圖片外鏈必須使用 http 或 https，且每個訂閱最多 10 個。",
    "subscription_image_admin_notice" => "你目前是管理員，可以上傳伺服器圖片，並可為本次上傳決定是否壓縮。為了安全，檔案仍會重新編碼。",
    "subscription_image_trusted_notice" => "受信使用者可以為每個訂閱上傳 1 張伺服器圖片。為了保護硬碟與頻寬，壓縮會強制保持開啟。",
    "subscription_image_free_notice" => "Free 使用者不能上傳伺服器圖片，但仍可填寫最多 10 個圖片外鏈。如需上傳權限，請聯絡 tcy 手動調整為受信使用者。",
    "subscription_image_compress_toggle" => "壓縮上傳到伺服器的圖片",
    "subscription_image_no_upload_permission" => "你目前的帳戶分組不允許為訂閱上傳伺服器圖片。",
    ...
    "subscription_image_upload_limit_dynamic" => "目前這個訂閱最多只能上傳 %d 張伺服器圖片。",
    "subscription_deleted" => "訂閱已刪除。",
    "subscription_image_limits_notice_dynamic" => "伺服器託管圖片單張上限 %d MB。允許的副檔名/格式：%s。圖片外鏈必須使用 http 或 https，且每個訂閱最多 %d 個。",
    "subscription_image_trusted_notice_dynamic" => "受信使用者可以為每個訂閱上傳最多 %d 張伺服器圖片。為了保護硬碟與頻寬，壓縮會強制保持開啟。",
    "subscription_image_free_notice_dynamic" => "Free 使用者不能上傳伺服器圖片，但仍可填寫最多 %d 個圖片外鏈。如需上傳權限，請聯絡 tcy 手動調整為受信使用者。",
    ...
    "subscription_image_storage_info" => "訂閱圖片會保存在 Docker 持久化掛載的圖片目錄中，並在資料庫裡另外記錄索引，重建容器後仍會保留。",
    "subscription_image_viewer_title" => "圖片預覽",
    "subscription_image_open_original" => "開啟原圖",
    "subscription_image_download" => "下載圖片",
    "subscription_image_no_selection" => "目前還沒有選取伺服器圖片。",
    ...
    "homepage_upload_policy_admin" => "你目前是管理員帳戶，可以上傳伺服器圖片，並可依本次上傳自行決定是否壓縮。",
    "homepage_upload_policy_trusted" => "你目前是受信使用者，可以為每個訂閱上傳 1 張伺服器圖片，系統會自動強制壓縮以保護儲存資源。",
    "homepage_upload_policy_free" => "你目前是 Free 使用者，仍可為每個訂閱填寫最多 10 個圖片外鏈，但伺服器圖片上傳會保持關閉，直到 tcy 手動為你提升權限。",
    // 設定頁面
    "monthly_budget" => "每月預算",

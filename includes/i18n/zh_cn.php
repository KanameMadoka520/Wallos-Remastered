...
$i18n = [
    ...
    "user_group_management_info" => "只有管理员可以调整用户分组。Free 用户只能使用图片外链，受信用户还可以为订阅详情上传托管到服务器的图片。",
    "subscription_images" => "订阅图片",
    "subscription_image_upload" => "上传服务器图片",
    ...
    "subscription_image_limits_notice" => "服务器托管图片单张上限 10 MB，仅支持 JPEG、PNG、WebP。图片外链必须使用 http 或 https，且每个订阅最多 10 个。",
    "subscription_image_admin_notice" => "你当前是管理员，可以上传服务器图片，并可为本次上传决定是否压缩。为安全起见，上传文件仍会重新编码。",
    "subscription_image_trusted_notice" => "受信用户可以为每个订阅上传 1 张服务器图片。为了保护硬盘和带宽，压缩会强制保持开启。",
    "subscription_image_free_notice" => "Free 用户不能上传服务器图片，但仍可填写最多 10 个图片外链。如需上传权限，请联系 tcy 手动调整为受信用户。",
    "subscription_image_compress_toggle" => "压缩上传到服务器的图片",
    "subscription_image_no_upload_permission" => "你当前的账户分组不允许为订阅上传服务器图片。",
    ...
    "subscription_image_upload_limit_dynamic" => "当前订阅最多只能上传 %d 张服务器图片。",
    "subscription_deleted" => "订阅已删除。",
    "subscription_image_limits_notice_dynamic" => "服务器托管图片单张上限 %d MB。允许的后缀/格式：%s。图片外链必须使用 http 或 https，且每个订阅最多 %d 个。",
    "subscription_image_trusted_notice_dynamic" => "受信用户可以为每个订阅上传最多 %d 张服务器图片。为了保护硬盘和带宽，压缩会强制保持开启。",
    "subscription_image_free_notice_dynamic" => "Free 用户不能上传服务器图片，但仍可填写最多 %d 个图片外链。如需上传权限，请联系 tcy 手动调整为受信用户。",
    ...
    "subscription_image_storage_info" => "订阅图片会保存在 Docker 持久化挂载的图片目录中，并在数据库里单独记录索引，重建容器后仍会保留。",
    "subscription_image_viewer_title" => "图片预览",
    "subscription_image_open_original" => "打开原图",
    "subscription_image_download" => "下载图片",
    "subscription_image_no_selection" => "当前还没有选中服务器图片。",
    ...
    "homepage_upload_policy_admin" => "你当前是管理员账户，可以上传服务器图片，并可按本次上传自行决定是否压缩。",
    "homepage_upload_policy_trusted" => "你当前是受信用户，可以为每个订阅上传 1 张服务器图片，系统会自动强制压缩以保护存储资源。",
    "homepage_upload_policy_free" => "你当前是 Free 用户，仍可为每个订阅填写最多 10 个图片外链，但服务器图片上传会保持关闭，直到 tcy 手动为你提升权限。",
    // 设置页面
    "monthly_budget" => "每月预算",

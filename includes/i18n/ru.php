...
$i18n = array_replace($i18n, [
    ...
    "allow_standard_users_local_webhooks" => "Разрешить обычным пользователям использовать список разрешённых адресов для вебхуков",
    "allow_standard_users_local_webhooks_info" => "Риск безопасности: включение этой опции позволит любому обычному (не администратору) пользователю отправлять уведомления на адреса из списка разрешённых адресов для вебхуков выше, включая внутренние службы вашей сети. Включайте это только если вы доверяете всем пользователям этого экземпляра Wallos.",
]);

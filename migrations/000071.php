...
$wallosMigration71TableColumns = function ($db, $tableName) {
...
$wallosMigration71CanCreateIndex = function ($db, $tableName, array $requiredColumns) use ($wallosMigration71TableColumns) {
    $columns = $wallosMigration71TableColumns($db, $tableName);
    if (empty($columns)) {
        return false;
    }
...
foreach ($wallosMigration71Indexes as $indexDefinition) {
    [$tableName, $indexName, $columns] = $indexDefinition;
    if (!$wallosMigration71CanCreateIndex($db, $tableName, $columns)) {
        continue;
    }

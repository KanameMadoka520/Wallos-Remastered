(function () {
  ...
  function renderBackupProgress(status) {
    ...
    const toneValue = String(
      status?.tone || (state === 'completed' ? 'success' : state === 'failed' ? 'error' : 'pending')
    );
  ...
  function backupDB() {
    ...
    renderBackupProgress({
      ...
      tone: 'pending',
      progress: 1,
      message: progressCard.dataset.startingMessage || translate('backup'),
    });
    ...
    postJson('endpoints/admin/createbackup.php', { operationId })
      ...
      .catch((error) => {
        ...
        showErrorMessage(normalizeBackupError(error, translate('backup_failed')));
      })
      .finally(() => {
        stopPolling();
  ...
  function verifyBackup(backupName, button) {
    ...
    postJson('endpoints/admin/verifybackup.php', { name: backupName })
      ...
      .catch((error) => {
        ...
        showErrorMessage(normalizeBackupError(error));
      })
      .finally(() => {
        button.disabled = false;
      });
  ...
  function restoreBackup(backupName, button) {
    ...
    if (!confirm(confirmMessage) || !confirm(confirmSecondMessage)) {
      return;
    }
  ...
  window.WallosAdminBackups = {
    backupDB,
    runPostRestoreActions,
    updateBackupCardStatus,
    verifyBackup,
    restoreBackup,
    openRestoreDBFileSelect,
    restoreDB,
    saveBackupSettingsButton,
    cleanupOldBackupsButton,
  };
})();

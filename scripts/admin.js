...
function formatAdminSubscriptionImageUserIssueSummary(audit, limit = 3) {
  const groups = Array.isArray(audit?.user_issue_summary) ? audit.user_issue_summary : [];
  if (groups.length === 0) {
    return adminTranslateWithFallback("subscription_image_audit_no_affected_users", "No affected users");
  }

  return groups.slice(0, limit).map((group) => {
    const userId = Number(group?.user_id ?? 0);
    const label = userId > 0
      ? `${adminTranslateWithFallback("user_id", "User ID")} ${userId}`
      : adminTranslateWithFallback("subscription_image_audit_unknown_user", "Unknown User");
    return `${label}: ${formatAdminNumber(group?.total_issues ?? 0)}`;
  }).join(" · ");
}
...
function renderAdminMaintenanceStorageSummary(storage) {
  ...
  const cards = [
    renderAdminMaintenanceStorageCard(
      adminTranslateWithFallback("storage_usage_generated_at", "Storage Snapshot Generated At"),
      storage.generated_at || "-",
      adminTranslateWithFallback("storage_usage_generated_at_detail", "This timestamp is generated by the server when the storage scan finishes."),
      "fa-solid fa-clock"
    ),
    renderAdminMaintenanceStorageCard(
...
function exportAdminSubscriptionImageAuditCsv() {
  ...
  const rows = [
    ...
    ["summary", "generated_at", audit.generated_at || "", "", "", ""],
    ...
    ["summary", "reclaimable_size_estimate", "", audit.reclaimable_bytes_estimate ?? 0, audit.reclaimable_size_estimate_label || "", ""],
  ];
  ...
  if (Array.isArray(audit.user_issue_summary)) {
    audit.user_issue_summary.forEach((item) => {
      rows.push([
        "affected_user",
        `user-${item?.user_id || "unknown"}`,
        item?.total_issues ?? 0,
        item?.orphan_bytes ?? 0,
        item?.orphan_size_label || "",
        [
          `orphan=${item?.orphan_files ?? 0}`,
          `missing_original=${item?.missing_original_rows ?? 0}`,
          `missing_variant=${item?.missing_variant_files ?? 0}`,
          `oversized=${item?.oversized_variants ?? 0}`,
        ].join("; "),
      ]);
    });
  }
...
function formatAdminMaintenanceAudit(audit) {
  ...
  const lines = [
    `${adminTranslateWithFallback("generated_at", "Generated At")}: ${audit.generated_at || "-"}`,
    ...
    `${adminTranslateWithFallback("subscription_image_reclaimable_size_estimate", "Estimated Reclaimable Size")}: ${audit.reclaimable_size_estimate_label || "0 B"}`,
    `${adminTranslateWithFallback("subscription_image_audit_affected_users", "Affected Users")}: ${Array.isArray(audit.user_issue_summary) ? audit.user_issue_summary.length : 0}`,
  ];
  ...
  if (Array.isArray(audit.user_issue_summary) && audit.user_issue_summary.length > 0) {
    lines.push("");
    lines.push(adminTranslateWithFallback("subscription_image_audit_affected_users", "Affected Users") + ":");
    audit.user_issue_summary.slice(0, 10).forEach((item) => {
      lines.push(`- user-${item.user_id || "?"}: ${item.total_issues || 0} · orphan=${item.orphan_files || 0} · missing_original=${item.missing_original_rows || 0} · missing_variant=${item.missing_variant_files || 0} · oversized=${item.oversized_variants || 0}`);
    });
  }
...
function renderAdminSubscriptionImageAuditSummary(audit) {
  ...
  const cards = [
    ...
    renderAdminSubscriptionImageAuditCard(
      adminTranslateWithFallback("subscription_image_audit_generated_at", "Audit Generated At"),
      audit.generated_at || "-",
      adminTranslateWithFallback("subscription_image_audit_generated_at_detail", "This timestamp is generated by the server when the scan finishes."),
      "ok",
      "fa-solid fa-clock"
    ),
    renderAdminSubscriptionImageAuditCard(
      adminTranslateWithFallback("subscription_image_audit_reclaimable_status", "Estimated Reclaimable Size"),
      audit.reclaimable_size_estimate_label || "0 B",
      adminTranslateWithFallback("subscription_image_audit_reclaimable_detail", "Estimated from orphan files and oversized derived variants."),
      Number(audit.reclaimable_bytes_estimate ?? 0) > 0 ? "watch" : "ok",
      "fa-solid fa-hard-drive"
    ),
    renderAdminSubscriptionImageAuditCard(
      adminTranslateWithFallback("subscription_image_audit_affected_users", "Affected Users"),
      formatAdminNumber(Array.isArray(audit.user_issue_summary) ? audit.user_issue_summary.length : 0),
      formatAdminSubscriptionImageUserIssueSummary(audit, 4),
      Array.isArray(audit.user_issue_summary) && audit.user_issue_summary.length > 0 ? "watch" : "ok",

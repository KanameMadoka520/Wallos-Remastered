...
- 用户封禁名单 / 计划删除时间管理
- 访问日志可折叠查看
- 访问日志记录请求耗时、HTTP 状态码和完成时间，并在后台显示最近慢请求指标
- 管理后台“慢请求接口排行”会按请求方法和路径聚合最近 24 小时慢请求，显示次数、平均耗时、最大耗时、总耗时、失败次数、失败率和最近出现时间，并可一键打开该接口的访问日志筛选结果
- 管理后台“系统总览”会集中显示服务健康、维护建议状态、SQLite 索引、慢请求、安全异常、订阅图片一致性和日志增长风险，并支持不整页刷新的局部刷新
- 管理后台维护建议面板，会把索引健康、数据库空闲页、孤儿图片、日志增长、慢请求和备份目录体积汇总成可执行建议
- 管理后台存储占用概览会显示访问日志、安全异常和速率限制用量的保留天数、最近 24 小时行数、保留窗口日均行数和增长风险
- 登录限速阈值后台可配置
- 顶部版本说明文字可在管理员后台编辑
...
- SQLite 忙/锁定时统一返回 `database_busy` JSON，并提示用户稍后重试，避免显示未知错误
- 请求日志记录与清理
- 图片直链受保护
- 备份直链受保护
- 自动备份 `db + logos`
...
- 管理员后台提供维护入口：
  ...
  - 查看访问日志、安全异常、速率限制用量表的增长风险、最近 24 小时行数和保留窗口日均行数
  - 扫描订阅图片孤儿文件和缺失衍生图记录
  - 通过订阅图片一致性状态卡查看索引覆盖、孤儿文件、缺失原图/派生文件、过大派生图和受影响用户 ID
  - 导出订阅图片审计 CSV，便于离线核对孤儿文件、缺失文件、过大派生图归属和体积
  - 记录管理员触发的维护动作审计日志，展示动作类型、管理员 ID、成功/失败、耗时和摘要，并支持刷新、摘要卡片、失败/慢动作筛选、复制摘要与按当前筛选导出 CSV
  - 一键清理数据库不再引用的订阅图片孤儿文件，范围限定在订阅图片目录内
  - 一键将大于原图的预览图/缩略图改为复用原图，并清理不再被引用的过大派生文件
  - 为订阅分页、筛选排序、实际支付记录、订阅图片索引、访问日志、安全异常和速率限制用量补充 SQLite 高频查询索引
  - 一键检查关键 SQLite 索引是否真实存在，避免迁移记录存在但索引缺失的隐性问题
  - 手动执行 SQLite `PRAGMA optimize` / `ANALYZE` / `VACUUM`，并显示维护前后数据库大小、页数和空闲页变化
...
- 管理后台系统总览契约：安全设置页应持续展示服务健康、维护状态、SQLite 索引、慢请求、安全异常、订阅图片和日志增长卡片，并保留局部刷新按钮
- 慢请求观测契约：请求日志耗时回写、慢请求统计、慢请求快捷筛选和 CSV 导出字段
- 慢接口排行契约：运行可观测性应持续返回 `top_slow_request_groups`，后台应展示按接口聚合的次数、平均/最大耗时、失败次数、最近出现时间和一键筛选日志入口
- 维护建议契约：后台应持续展示正常/关注/建议处理状态，并能从建议卡片触发安全的维护动作或打开对应筛选视图
- 管理员后台 E2E：真实打开后台并点击运行诊断刷新、维护建议刷新、访问日志/异常弹窗、缓存刷新提示和备份校验流程
...
订阅图片链路有独立浏览器级 E2E，适合在修改上传、预览、图片权限、缩略图/预览图派生策略后运行：
